...
SessionSchema.methods.populateFromJSON = function populateFromJSON(json, callback) {
	...
	self.class_begin_date = Date.parse(json.CLASS_BEGIN_DATE) || null;
	self.last_add_drop_date = Date.parse(json.LAST_ADD_DROP_DATE) || null;
	self.withdraw_with_w_date = Date.parse(json.WITHDRAW_W_W_DATE) || null;
	self.final_exam_begin_date = Date.parse(json.FINAL_EXAM_BEGIN_DATE) || null;
	self.final_exam_end_date = Date.parse(json.FINAL_EXAM_END_DATE) || null;
	self.class_end_date = Date.parse(json.CLASS_END_DATE) || null;
	self.stop_date = Date.parse(json.STOP_DATE) || null;
